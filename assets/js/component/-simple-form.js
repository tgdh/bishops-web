...
*/		
	...
	stepsForm.prototype._init = function() {
		...
		// questions
		this.questions = [].slice.call( this.el.querySelectorAll( '.contourField' ) );

		// total questions
		this.questionsCount = this.questions.length;

		// get wrap
		this.formWrap = this.el.querySelector( '.contourPage' );

		// add form styling class
		classie.addClass( this.el, 's-simple-form' );

		// show first question
		classie.addClass( this.questions[0], 'current' );

		// add no validate to form
		this.el.querySelector('form').setAttribute('novalidate','novalidate');

		this._controls();

		// next question control
		this.ctrlNext = this.el.querySelector( '.s-simple-form__next' );
		this.ctrlNext.setAttribute( 'aria-label', 'Next' );


		// progress bar
		this.progress = this.el.querySelector( '.s-simple-form__progress' );
		...
		this.questionStatus = this.el.querySelector( '.s-simple-form__numbers' );
		...
		this.currentNum = this.questionStatus.querySelector( '.s-simple-form__numbers-current' );
		this.currentNum.innerHTML = Number( this.current + 1 );
		// total questions placeholder
		this.totalQuestionNum = this.questionStatus.querySelector( '.s-simple-form__numbers-total' );
		this.totalQuestionNum.innerHTML = this.questionsCount;

		// error message
		this.error = this.el.querySelector( '.s-simple-form__error-message' );
	...
	// create controls
	stepsForm.prototype._controls = function() {
		// first, create next question number placeholder
		this.nextBtn = document.createElement( 'button' );
		this.nextBtn.className = 's-simple-form__next ico-arrow-right';

		this.progressBar = document.createElement( 'div' );
		this.progressBar.className = 's-simple-form__progress';

		this.numEl = document.createElement( 'span' );
		this.numEl.className = 's-simple-form__numbers';

		this.numElCurrent = document.createElement( 'span' );
		this.numElCurrent.className = 's-simple-form__numbers-current';

		this.numElTotal = document.createElement( 'span' );
		this.numElTotal.className = 's-simple-form__numbers-total';

		this.numEl.appendChild( this.numElCurrent );
		this.numEl.appendChild( this.numElTotal );

		this.errors = document.createElement( 'span' );
		this.errors.className = 's-simple-form__error-message';

		this.finalMessage = document.createElement( 'span' );
		this.finalMessage.className = 's-simple-form__final-message';

		// insert it in the DOM
		this.formWrap.appendChild( this.nextBtn );
		this.formWrap.appendChild( this.numEl );
		this.formWrap.appendChild( this.progressBar );
		this.formWrap.appendChild( this.errors );
		this.el.appendChild( this.finalMessage );
...
//		this.el.querySelector('input[type="submit"]').click();
	...
	//
	stepsForm.prototype._reviewForm = function() {
	...
	// check if errors
	stepsForm.prototype._checkErrors = function() {
		var selection = document.querySelector('.js-simple-form').querySelectorAll('.field-validation-error');
